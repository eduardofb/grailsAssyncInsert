	...
	<body>
		...
		<div id="list-quarto" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="nome" title="${message(code: 'quarto.nome.label', default: 'Nome')}" />
					
					</tr>
				</thead>
				<tbody>
				<g:each in="${quartoInstanceList}" status="i" var="quartoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
					
						<td><g:link action="show" id="${quartoInstance.id}">${fieldValue(bean: quartoInstance, field: "nome")}</g:link></td>
			...
			<div class="pagination">
				<g:paginate total="${quartoInstanceTotal}" />
			</div>
		</div>
	</body>
</html>
